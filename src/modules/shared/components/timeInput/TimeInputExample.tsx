...
const timeSchema = yup.object({
  ...
  meetingDateTime: yup.date().required('Data e hora da reunião são obrigatórios'),
}).test('endTime', 'Horário de fim deve ser depois do início', function(value) {
...
const TimeInputExample = () => {
  ...
  return (
    <View className="p-4 space-y-4">
      ...
      <View>
        <Text className="text-lg font-semibold mb-2">TimeInput com Validação</Text>
      ...
      {/* Disabled TimeInput */}
      <View>
        <Text className="text-lg font-semibold mb-2">TimeInput Desabilitado</Text>
        <TimeInput
      ...
      <View className="mt-6 p-4 bg-gray-100 rounded-lg">
        ...
        <Text>Início: {watchedValues.startTime ? watchedValues.startTime.toLocaleTimeString() : 'Não selecionado'}</Text>
        <Text>Fim: {watchedValues.endTime ? watchedValues.endTime.toLocaleTimeString() : 'Não selecionado'}</Text>
        <Text>Data: {watchedValues.appointmentDate ? watchedValues.appointmentDate.toLocaleDateString() : 'Não selecionado'}</Text>
        <Text>Data/Hora: {watchedValues.meetingDateTime ? watchedValues.meetingDateTime.toLocaleString() : 'Não selecionado'}</Text>
      </View>
